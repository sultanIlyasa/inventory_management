<template>
    <div class="bg-white border rounded-lg shadow-lg" :class="containerClass">
        ...
        <div class="p-2 sm:p-3 lg:p-4 border-b">
            <div class="text-center sm:text-left">
                ...
                <p v-if="!isCompact" class="text-xs sm:text-sm text-gray-600 mt-1">
                    Materials with longest caution streaks
                </p>
            </div>
        </div>
        ...
        <div v-if="!isCompact" class="grid grid-cols-2 gap-2 sm:gap-3 lg:gap-4 p-2 sm:p-3 lg:p-4 bg-gray-50">
            <div class="bg-red-100 border-l-2 sm:border-l-4 border-orange-600 p-2 sm:p-3 rounded-lg">
                <div class="text-xs sm:text-sm text-orange-700 font-semibold">Total Caution</div>
            ...
            <div class="bg-yellow-100 border-l-2 sm:border-l-4 border-yellow-500 p-2 sm:p-3 rounded-lg">
                <div class="text-xs sm:text-sm text-yellow-700 font-semibold">Average Streak</div>
                <div class="text-lg sm:text-xl lg:text-2xl font-bold text-yellow-900">
                ...
                <tbody>
                    <tr v-for="(item, index) in currentLeaderboard" :key="item.material_number"
                        ...
                        <td class="p-1.5 sm:p-2 lg:p-3 border text-center">
                            <span
                                class="inline-block px-1.5 sm:px-2 py-0.5 sm:py-1 rounded text-[9px] sm:text-[10px] lg:text-xs font-semibold bg-orange-500 text-white whitespace-nowrap">
                                {{ item.current_status }}
                            </span>
                        </td>
                    ...
                    <tr v-if="!currentLeaderboard.length">
                        <td colspan="5" class="p-4 sm:p-6 text-center text-gray-500">
                            ...
                            <div class="text-xs sm:text-sm">No materials in CAUTION status</div>
                        </td>
                    </tr>
                </tbody>
            </table>
...
const props = defineProps({
    ...
    initialStatistics: {
        type: Object,
        default: () => ({
            total: 0,  // ✅ Changed from total_caution
            average_days: 0,
            max_days: 0,
            min_days: 0
        })
    ...
    initialPagination: {
        ...
        default: () => ({
            ...
            last_page: 1,
            per_page: 10,
            total: 0  // ✅ Changed from total_page
        })
    },
    size: {
        type: String,
    ...
    refreshInterval: {
        type: Number,
        default: 30000  // ✅ Changed to 30 seconds
    },
    hideRefresh: {
        type: Boolean,
        default: false
...
const leaderboard = ref([...props.initialLeaderboard]);  // ✅ Clone array
const statistics = ref({ ...props.initialStatistics });  // ✅ Clone object
const pagination = ref({ ...props.initialPagination });  // ✅ Clone object
const isLoading = ref(false);
let intervalId = null;

// ✅ Fixed computed properties
const currentLeaderboard = computed(() =>
    leaderboard.value.length > 0 ? leaderboard.value : props.initialLeaderboard
);

const currentStatistics = computed(() =>
    statistics.value.total !== undefined ? statistics.value : props.initialStatistics
);
...
const fetchLeaderboard = async (page = 1) => {
    ...
    try {
        ...
        const response = await axios.get(route('warehouse-monitoring.api.caution'), { params });
...
watch(() => props.initialStatistics, (newVal) => {
    if (newVal && newVal.total !== undefined) {
