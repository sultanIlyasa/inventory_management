    ...
    <div class="bg-white border rounded-lg shadow" :class="containerClass">
        <div class="p-3 sm:p-4 border-b text-center sm:text-left">
            ...
            <p v-if="!isCompact" class="text-xs text-gray-600 mt-1">
                Materials with the longest SHORTAGE streaks
            </p>
        </div>
        ...
        <div v-if="!isCompact" class="grid grid-cols-2 gap-3 p-3 bg-gray-50">
            <div class="rounded-lg border-l-4 border-red-500 bg-red-50 p-3">
                <div class="text-xs text-red-700 font-semibold">Total Shortage</div>
                <div v-if="isLoading" class="mt-1 h-8 w-16 animate-pulse rounded bg-red-200"></div>
                <div v-else class="text-2xl font-bold text-red-900">{{ currentStatistics.total }}</div>
            </div>
            <div class="rounded-lg border-l-4 border-orange-500 bg-orange-50 p-3">
                <div class="text-xs text-orange-700 font-semibold">Average Streak</div>
                <div v-if="isLoading" class="mt-1 h-8 w-24 animate-pulse rounded bg-orange-200"></div>
                <div v-else class="text-2xl font-bold text-orange-900">{{ currentStatistics.average_days }} days</div>
            </div>
        </div>
        ...
        <div v-if="isCompact" class="p-2">
            ...
            <a v-if="showViewAll && currentPagination.total > limitValue" :href="viewAllUrl"
                class="mt-3 block text-center text-xs font-semibold text-red-600 hover:text-red-800">
                    ...
                    <template v-else>
                        ...
                        <tr v-if="!currentLeaderboard.length">
                            <td colspan="6" class="p-6 text-center text-gray-500">
                                ...
                                <p class="text-sm font-semibold">No materials in SHORTAGE status</p>
                            </td>
                        </tr>
                    </template>
                </tbody>
                ...
                <div class="flex flex-wrap gap-2">
                    ...
                    <button v-for="page in visiblePages" :key="page" @click="emitPage(page)"
                        :class="['px-3 py-1.5 rounded border text-sm',
                            currentPagination.current_page === page ? 'bg-red-500 text-white border-red-500' : 'hover:bg-gray-100']">
                        {{ page }}
                    </button>
                    <button class="px-3 py-1.5 rounded border text-sm hover:bg-gray-100 disabled:opacity-50"
                        @click="emitPage(currentPagination.current_page + 1)"
        ...
        <div v-if="!hideRefresh" class="border-t p-3 flex justify-end">
            <button @click="emitRefresh"
                class="rounded bg-red-600 px-4 py-2 text-xs font-semibold text-white hover:bg-red-700">
                Refresh Data
            </button>
        </div>
    </div>
...
const props = defineProps({
    ...
    viewAllUrl: {
        type: String,
        default: '/warehouse-monitoring/leaderboard?tab=SHORTAGE'
    },
    hideRefresh: {
        type: Boolean,
        default: false
