<template>
    <div class="bg-white rounded-lg shadow" :class="containerClass">
        ...
        <div v-if="!isCompact && statistics" class="grid grid-cols-2 sm:grid-cols-4 gap-3 p-3 sm:p-4 bg-gray-50">
            <div class="text-center p-3 bg-red-100 rounded-lg">
                <div class="text-sm text-red-700 font-semibold">To Caution</div>
                <div class="text-2xl font-bold text-red-900">{{ statistics.total_to_caution }}</div>
            </div>
            <div class="text-center p-3 bg-orange-100 rounded-lg">
                <div class="text-sm text-orange-700 font-semibold">To Shortage</div>
                <div class="text-2xl font-bold text-orange-900">{{ statistics.total_to_shortage }}</div>
            </div>
            <div class="text-center p-3 bg-blue-100 rounded-lg">
                <div class="text-sm text-blue-700 font-semibold">To Overflow</div>
                <div class="text-2xl font-bold text-blue-900">{{ statistics.total_to_overflow }}</div>
            </div>
            <div class="text-center p-3 bg-green-100 rounded-lg">
                <div class="text-sm text-green-700 font-semibold">Recovered</div>
                <div class="text-2xl font-bold text-green-900">{{ statistics.total_recovered }}</div>
            </div>
        </div>

        <!-- Compact View (List) -->
        <div v-if="isCompact" class="p-3">
            <div class="space-y-2">
                <div v-for="(item, index) in limitedItems" :key="item.material_number"
                    ...
                    <div class="flex items-center gap-2 flex-1 min-w-0">
                        ...
                        <div class="min-w-0 flex-1">
                            <div class="font-semibold text-sm truncate">{{ item.material_number }}</div>
            ...
            <!-- View All Link -->
            <a v-if="showViewAll && materials.length > limit" :href="viewAllUrl"
            ...
            <table class="min-w-full text-xs sm:text-sm border-collapse">
                <thead class="bg-gray-100 text-gray-900 uppercase text-[10px] sm:text-xs font-semibold sticky top-0">
                    <tr>
                        ...
                        <th class="p-2 sm:p-3 border text-center bg-green-50">Recovered</th>
                        <th class="p-2 sm:p-3 border text-center bg-gray-50 font-bold">Total Changes</th>
                    </tr>
                </thead>
                <tbody>
                    <tr v-for="item in sortedMaterials" :key="item.material_number"
                        ...
                        <td class="p-2 sm:p-3 border text-center">
                            <div class="text-lg font-bold text-green-600">
                                {{ item.frequency_changes.total_to_ok }}
                            </div>
                        </td>
                    ...
                    <!-- Empty State -->
                    <tr v-if="!sortedMaterials.length">
...
const sortedMaterials = computed(() => {
    const sorted = [...materials.value].sort((a, b) => {
        switch (props.sortBy) {
            case 'caution':
                return b.frequency_changes.ok_to_caution - a.frequency_changes.ok_to_caution
            case 'shortage':
                return b.frequency_changes.ok_to_shortage - a.frequency_changes.ok_to_shortage
            case 'overflow':
                return b.frequency_changes.ok_to_overflow - a.frequency_changes.ok_to_overflow
            case 'total':
            default:
                return b.frequency_changes.total_from_ok - a.frequency_changes.total_from_ok
        }
    })
    return sorted
})
...
const limitedItems = computed(() => {
    ...
    return sortedMaterials.value.slice(0, itemLimit)
})

const statistics = computed(() => {
    if (materials.value.length === 0) return null

    return {
        total_to_caution: materials.value.reduce((sum, m) => sum + m.frequency_changes.ok_to_caution, 0),
        total_to_shortage: materials.value.reduce((sum, m) => sum + m.frequency_changes.ok_to_shortage, 0),
        total_to_overflow: materials.value.reduce((sum, m) => sum + m.frequency_changes.ok_to_overflow, 0),
        total_recovered: materials.value.reduce((sum, m) => sum + m.frequency_changes.total_to_ok, 0),
    }
})
...
const fetchData = async () => {
    ...
    try {
        const res = await axios.get('/api/reports/general')
